...
const PAGE_SIZE = 10;
const LIST_URL = '/user/packaging/standard-forms-list';
...
export interface StandardFormsListSearchQueryFilter {
  isCreatedSoloReport: string;
  fieldOperation: string;
}

export interface GetStandardFormsListParams {
  pageParam: number;
  searchBoxList: SearchBoxItem[];
  filterBoxList: SearchBoxItem[];
  searchQueryFilter: StandardFormsListSearchQueryFilter;
}
...
export async function getStandardFormsListAction({
  pageParam,
  searchBoxList,
  filterBoxList,
  searchQueryFilter,
}: GetStandardFormsListParams) {

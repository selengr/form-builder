...
const fetchData = async (id: string | string[]) => {
    const customComboFilterModel = {
        ...
        rows: 10000,
        extMap: {
            formId: id, typeRequest: "ONLY_ALL_CALC"
        }
    };
...
export const useGetOnlyAllCalculation = () => {
    ...
    const onlyAllCalculationOptions = data?.dataList?.map((item: IConditionQuestionType) => ({
        value: `${item?.extMap.UNIC_NAME}@${item.caption}`, label: item.caption,
    }));


    return {
        isFetchingOnlyAllCalculation: isFetching, onlyAllCalculation: data?.dataList, onlyAllCalculationOptions
    };
};

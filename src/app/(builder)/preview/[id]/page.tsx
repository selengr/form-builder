...
export default function PreviewPage() {
  ...
  const builderBackHref = getBuilderBackHref(paramId as string, from, admin);

  const handleBack = () => {
    router.push(builderBackHref);
  };
    ...
    return (
      <div className="w-full h-screen flex justify-center items-center px-4">
        <div className="max-w-md w-full bg-white rounded-xl shadow p-6 text-center">
          ...
          <Button
            variant="contained"
            onClick={() => router.push(builderBackHref)}
            sx={{
              backgroundColor: '#111',
  ...
  return (
    <div
      className={`w-full flex flex-col overflow-hidden h-[calc(100dvh-76px)] md:h-screen p-2 sm:p-3`}
    >
      <div className={`flex flex-col bg-white rounded-xl overflow-hidden h-full`}>
        <div className="shrink-0 m-2 p-4 z-10 w-[calc(100%-16px)]  h-[52px] flex items-center justify-center rounded-lg bg-[#F7F7FF] mb-4 relative">
          <IconButton
            sx={{ position: 'absolute', left: { xs: '2px', sm: '8px' } }}
            onClick={handleBack}
          >

...
export default function GroupsPage() {
    ...
    const fetchGroups = useCallback(async () => {
        ...
        try {
            ...
            const transformedGroups: IGroup[] = data.content.map(item => ({
                id: item.groupId, name: item.groupName, description: '', userCount: item.groupMemberCount,
            }));
    ...
    return (<div className="p-2 w-full h-[calc(100vh - 60px)] md:h-screen flex flex-col" draggable={false}>
            <main className="p-4 bg-white flex flex-col rounded-xl h-full">
                <div
                    className="min-h-[52px] flex items-center justify-center relative rounded-xl bg-[#F7F7FF] mb-4 px-2">
                ...
                <div className="flex justify-center items-center mb-2">
                    <div className="flex w-full max-w-lg items-center">
                        <div
                            className="flex-1 bg-[#ECFAFF] rounded-xl px-4 py-3.5 flex justify-between items-center ml-2">
                ...
                <div className="flex justify-center flex-1 overflow-y-auto pb-6 min-h-0">
                    {loading ? (<p className="text-gray-600">در حال بارگذاری گروه‌ها...</p>) : error ? (
                        <p className="text-red-500">خطا در بارگذاری گروه‌ها: {error}</p>) : groups.length === 0 ? (
            ...
            {showCreateGroupDialog && (<CreateGroupDialog
                    onClose={() => router.back()}

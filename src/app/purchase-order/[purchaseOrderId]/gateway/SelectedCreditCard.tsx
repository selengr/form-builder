...
export function SelectedCreditCard({ availableAmount = 0, creditTypeValue, onDelete, remainedCredit = 0 }: SelectedCreditCardProps) {
  const { palette } = useTheme();

  return (
    <Box border='1px solid #DDE1E6' padding='0.7rem 0.6rem' marginY='0.5rem' borderRadius='12px'>
      <Box display='flex' alignItems='center' marginBottom='0.2rem'>
        ...
        <Typography variant='body1' fontWeight='bold' color={palette.primary.main}>
          {formatNumberWithCommas(Math.min(availableAmount, availableAmount - remainedCredit).toString())}
          تومان
        </Typography>
      </Box>
      <Box display='flex' alignItems='center' flexWrap='wrap'>
        ...
        <Typography variant='body1' fontWeight='bold' color={palette.primary.main} whiteSpace='nowrap'>

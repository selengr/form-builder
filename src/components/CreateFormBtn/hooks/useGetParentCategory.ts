...
const fetchCategoryData = async () => {
    const customComboFilterModel = {
        type: "COMBO", entity: "PROJECTS", input: "", page: 0, rows: 1000
    };
...
export const useGetParentCategory = () => {
    ...
    const {data, isFetching} = useQuery({
        queryKey: ['PARENT_CATEGORY'], queryFn: () => fetchCategoryData(), gcTime: 600000, staleTime: 0, retry: 3
    });

    const Category = data?.dataList
        ?.map((item: IGetCategory) => ({
            value: item.value, label: item.caption,
        }));


    return {
        isFetchingCategory: isFetching, Category,
    };
};

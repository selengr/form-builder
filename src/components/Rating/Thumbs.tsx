...
export default function ThumbsRating({
    ...
    const renderCustomIcon = (index: number) => {
        ...
        return (
            <div
                key={index}
                className={`relative cursor-pointer mx-[2px] group ${readOnly ? "cursor-default" : ""} ${isPopping ? "animate-pop" : ""}`}
                ...
                onMouseMove={(e) => {
                    ...
                    setHoverValue(
                        Math.round((index + percent) / precision) * precision
                    );
                }}
        ...
        >
            <div className="flex flex-col">
                <div className="flex justify-center items-center relative">
                    ...
                    {Array.from({ length: max }).map(renderCustomIcon)}
                    <span className="md:flex hidden justify-between mt-2">

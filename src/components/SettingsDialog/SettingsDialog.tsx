...
const responseLimitationOptions = [{label: "از طریق شماره همراه", value: "PHONE_NUMBER"}, {
    label: "از طریق ایمیل",
    value: "EMAIL"
},];

const layoutOptions = [{label: "نمایش فهرستی", value: "list-view"}, {label: "نمایش صفحه‌ای", value: "page-view"},];

const themeOptions = [{label: "تم 1", value: "theme_1"}];

const fieldsConfig = [{
    name: "timeToComplete", label: "زمان شروع", type: "time-picker",
}, {
    name: "expireDate", label: "تاریخ فعال سازی و انقضا فرم", type: "date-picker",
}, {
    name: "responseLimitation", label: "محدودیت پاسخ‌‌دهی", type: "select", options: responseLimitationOptions,
}, {
    name: "layout", label: "حالت نمایش", type: "multi-select", options: layoutOptions, disabled: true,
}, {
    name: "theme", label: "پوسته", type: "select", options: themeOptions, disabled: true,
},];
...
const propertiesSchema = z.object({
    name: z
        ...
        .pipe(z
            ...
            .max(100, {message: "حداقل باید 2 و حداکثر 100 کاراکتر باشد"})), responseLimitation: z.object({
        value: z.string(), checked: z.boolean(),
    }), layout: z.object({
        value: z.array(z.string()), checked: z.boolean(),
    }), theme: z.object({
        value: z.string(), checked: z.boolean(),
    }), expireDate: z.object({
        value: z.any(), checked: z.boolean(),
...
export default function SettingsDialog({formName, onChangeName}: Props) {
    ...
    async function onSubmit(values: propertiesFormSchemaType) {
        const body = {
            ...convertObject(values as any, fieldsConfig), name: formFieldName
        };
    ...
    return (<>
            <IconButton
                onClick={handleOpen}
                sx={{
                    height: "40px", width: "40px", display: "flex", justifyContent: "center", alignItems: "center",
                }}
            >
                            ...
                            <div dir="rtl" className="flex flex-col pb-4 p-2">
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        {fieldsConfig.map((field) => (<FieldSwitchPair
                                                key={field.name}
                                        ...
                                        <Button
                                            ...
                                            sx={{
                                                ...
                                                "&.MuiButtonBase-root:hover": {
                                                    bgcolor: "transparent", boxShadow: "none", color: "#1758BA",
                                                },
                                            }}
                                            variant="outlined"
                                            onClick={handleOpen}

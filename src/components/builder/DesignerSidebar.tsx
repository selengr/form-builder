...
const DesignerSidebar = memo(() => {
  ...
  const renderElements = useMemo(() => ELEMENTS.map((el, index) => <SidebarBtnElement key={index} formElement={el} disabled={formSetting.formStatus !== 'CREATE'} />), []);
  ...
  const PublishButton = (
    <Button
      ...
      <p className='text-white font-bold text-[15px]'>{formSetting.formStatus === 'CREATE' ? 'آماده برای انتشار' : 'منتشر شده'}</p>
    </Button>
  );

...
const AdvancedFormulaEditor: React.FC<IAdvancedFormulaEditorProps> = ({
  ...
  const handleOperator = (content: string) => {
    ...
    if (nextElement) {
      if ((nextElement.type === "PARENTHESIS" || nextElement.type === "AVG_PARENTHESIS") && nextElement.content === ")") {
    ...
    try {
      if (!isEdit) {
        ...
        await AxiosApi.put(`/calculation/${editList?.id}`, {
          id: editList?.id, name: formName, formBuilderId: id, theFormula: formula, frontCalcData: JSON.stringify(elements),
        });
      }
      handleClose();
      refresh();
      ...
      case "NEW_FIELD":
        return (
          ...
          <div
            ...
            data-type="NEW_FIELD"
          >
            <Tooltip title={elem.content} enterDelay={800} arrow>
  ...
  >
    ...
    >
      ...
      >
        ...
        <Box
          sx={{
            width: {xs: "100%", sm: "73%"}, display: "flex", flexDirection: "column", alignItems: "start",
          }}
        >
          <Typography
            variant="subtitle1"
      ...
      >
        <LoadingButton
          ...
          variant="contained"
          sx={{
            backgroundColor: "#1758BA", fontWeight: "500", fontSize: "15px", borderRadius: "8px", height: "50px", "&.MuiButtonBase-root:hover": {

...
export default function DesignerFinishPageElement() {
  ...
  return (
    <Fragment>
      {!finishPage ? (
        <div dir='rtl' className='flex px-3 flex-row w-full relative items-center justify-center rounded-xl bg-[#F7F7FF]'>
          <p className='text-base p-4 flex font-bold justify-center flex-grow'>صفحه پایان</p>
          <button
            onClick={() => {
        ...
        <div dir='rtl' className='flex flex-col w-full relative items-center justify-center p-2 rounded-xl bg-[#f7f7f7]'>
          <p className='text-base font-bold px-4 pb-4 pt-2 flex justify-center flex-grow'>صفحه پایان</p>
          <DesignerElementNoDnD element={finishPage} />
        </div>
      )}
    </Fragment>
  );

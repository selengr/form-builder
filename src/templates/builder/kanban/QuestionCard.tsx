...
const QuestionCard = memo(function QuestionCard({ question }: { question: FormElementInstance }) {
  const { setNodeRef, attributes, listeners, transform, transition, isDragging, index } = useSortable({
  ...
  if (question?.temp) {
    return <div className='flex items-center opacity-30 h-[65px] border-[1px] border-[#1758BA] rounded-xl bg-white' style={style}></div>;
  }
    ...
    <div className='relative w-full'>
      <div
        ...
        className={`flex items-center h-[65px] w-full relative justify-start flex-row p-2 border-[1px] rounded-xl bg-white ${
          isDragging ? 'border-[#CCC] opacity-50' : 'border-[#1758BA] opacity-100'
        }`}>

...
import {ICalculator, TRefetchFunction} from "@/types/calculator";
...
const CalculatorList: React.FC<ICalculatorListProps> = ({calculators, refetch}) => {
    ...
    return (<div className="w-full h-[calc(100vh-6rem)] max-w-md flex flex-col p-[13px] overflow-hidden">
        ...
        >
            {calculators?.map((calculator: ICalculator, index: number) => (
                <CalculatorCard index={index} calculator={calculator} key={index} refetch={refetch}

...
const CalculatorList: React.FC<ICalculatorListProps> = ({calculators}) => {
  ...
  return (<div className="w-full max-w-md flex flex-col p-[13px]">
      <CreateCalculator/>
      {calculators.length > 0 && (<div
        ...
        >
          {calculators?.map((calculator: ICalculator, index: number) => (
            <CalculatorCard index={index} calculator={calculator} key={index} disabled={formSetting.formStatus !== "CREATE"}/>))}
        </div>)}
    </div>);
};

...
export const getInput = (
  ...
  { onlySomeQuestionsOptions, isFetchingOnlyAllQuestions, onlyAllCalculationOptions, isFetchingOnlyAllCalculation, onlyAllQuestions, onlyAllDateOptions, control, setValue }: any,
) => {
  ...
  switch (combinedKey) {
    ...
    case 'MULTIPLE_CHOICE_OPTION_#greaterThanMultiChoiceSingle': {
      ...
      const targetQuestion = onlyAllQuestions?.find((item: IConditionQuestionType) => item?.extMap?.UNIC_NAME === targetUnicName);
      ...
      return <SelectController key={targetUnicName} name={field.name} options={mappedOptions} placeholder="مقدار" sx={{ minWidth: 215, width: '100%' }} />;
    }
    ...
    case 'TEXT_FIELD_DATE_QUESTION_#afterDate':
      return <SelectController name={field.name} options={onlyAllDateOptions} isLoading={isFetchingOnlyAllQuestions} placeholder="مقدار" sx={{ minWidth: 215, width: '100%' }} />;
    ...
    case 'INFO_FIELD_TEXT_#equalThanNumber':
      return <TextFieldController disabled name={field.name} type="number" sx={{ width: '100%' }} />;

    default:
      return <TextFieldController name={field.name} disabled sx={{ width: '100%' }} />;
  }
};

...
function CartItem({ detail, onSelect, onRemove, toggleConfirm, loading, open, index }: ICartItemProps) {
  ...
  return (
    <>
      <div onClick={onSelect} className={`flex items-center gap-4 p-4 rounded-2xl border border-neutral-200`}>
        <div className='flex-shrink-0 w-12 h-12 rounded-2xl bg-[#F4F6FB] text-blue-500 font-bold flex items-center justify-center'>{index + 1}</div>
        ...
        <div className='flex-grow flex flex-col gap-1'>
          ...
          <div className='mt-1 flex flex-col gap-1'>
            {purchaseOrderProductModels?.map((product) => (
              <span key={product.purchaseOrderProductId} className='text-sm text-gray-500 line-clamp-1'>

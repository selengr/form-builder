...
export const ConditionalSystem: React.FC<IConditionalSystemProps> = ({ handleClose, condition, isEdit = false }) => {
          ...
          subConditions
            .map((subCondition: TSubConditionData) => {
              ...
              if (operatorType === 'OPTION') {
                ...
                } else if (conditionType === '#lenEqualText' || conditionType === '#lenGreaterThanText' || conditionType === '!#lenGreaterThanText') {
                  formattedValue = `{#v_${value}}`;
                } else {
  ...
  useEffect(() => {
    if (condition?.returnText?.trim().startsWith("{")) {
         const errorMessage = "نسخه این گزارش‌ قدیمی است و دیگر پشتیبانی نمی‌شود. لطفاً یک گزارش جدید ایجاد کنید.";

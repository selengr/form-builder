...
const ActionButton = ({ label, Icon, onClick }: IProps) => (
  <button
    type="button"
    onClick={onClick}
    className="flex shrink-0 items-center justify-center h-8 rounded-lg bg-white text-[10px] md:text-[13px] md:font-medium px-1.5 md:px-2 cursor-pointer hover:bg-gray-100 transition-colors whitespace-nowrap">
...
export const RenderAction = ({
  ...
  name: string;
  setPublicationApprovalByAdmin: any;
}) => {
  ...
  return (
    <div className="w-full min-h-14 px-2 sm:px-3 md:px-4 py-2 bg-[#F7F7FF] rounded-lg flex justify-between items-center gap-1 sm:gap-2 shadow-sm border border-[#E8E8F5]">
      <div className="flex min-w-0 flex-1 gap-1 sm:gap-2 overflow-x-auto">
      ...
      <div className="flex min-w-0 flex-1 justify-end gap-1 sm:gap-2 overflow-x-auto">

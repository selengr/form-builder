...
const ActionButton = ({ label, Icon, onClick }: IProps) => (
  <button onClick={onClick} className='flex justify-center items-center h-8 rounded-lg bg-white text-[10px] md:text-[13px] md:font-medium md:px-2 cursor-pointer hover:bg-gray-100 transition-colors'>
...
export const RenderAction = ({ publicationApprovalByAdmin, name, setPublicationApprovalByAdmin }: { publicationApprovalByAdmin: boolean; name: string, setPublicationApprovalByAdmin : any }) => {
  ...
  return (
    <div className="w-full h-14 px-3 md:px-4 py-2 bg-[#F7F7FF] rounded-lg flex justify-between items-center shadow-sm border border-[#E8E8F5]">
      <div className="flex w-full gap-2 md:gap-3 overflow-x-auto">
      ...
      <div className="flex w-full gap-2 md:gap-3 justify-end overflow-x-auto">

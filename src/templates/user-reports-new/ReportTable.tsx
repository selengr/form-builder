          ...
          <thead className='sticky top-0 z-10 mb-2'>
            <tr>
              ...
              {headData.map((item, index) => (
                <Tooltip key={item.questionId} title={item.questionTitle} followCursor arrow enterDelay={1000} placement='top'>
                  <th
                    className={`
            ...
            {allData.map((row, rowIndex) => (
              <tr key={rowIndex} className={rowIndex % 2 !== 0 ? 'bg-[#F7F7FF]' : 'bg-white'}>
                ...
                <TableCell
                  content={
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-medium ${row.publicationApprovalByAdmin

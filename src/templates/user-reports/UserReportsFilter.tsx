...
export default function UserReportsFilter({
  mode,
  filter,
  onChange,
  onApply,
  onReset,
}: UserReportsFilterProps) {
      ...
      <div className="flex-1 overflow-y-auto pb-4 min-h-0" style={{ scrollbarWidth: 'thin' }}>
        <div className="w-full bg-[#F7F7FF] rounded-[20px] px-4 pt-4 pb-3 mb-4">
          <FormControl>
            ...
            <RadioGroup
              value={filter.responseForDestroyerReport ?? 'ALL'}
              onChange={(event) =>
                onChange((prev) => ({
                  ...prev,
                  responseForDestroyerReport: event.target.value,
                }))
              }>
        ...
        <div className="w-full bg-[#F7F7FF] rounded-[20px] px-4 pt-4 pb-3 mb-4">
          <FormControl>
            ...
            <RadioGroup
              value={filter.typeOfReport ?? 'ALL'}
              onChange={(event) =>
                onChange((prev) => ({ ...prev, typeOfReport: event.target.value }))
              }>
              <FormControlLabel value="ALL" control={<Radio />} label="همه" />
